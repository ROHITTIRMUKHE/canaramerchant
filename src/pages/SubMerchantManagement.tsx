...
import { subMerchantData } from '@/data/subMerchants';
...
export default function SubMerchantManagement() {
  ...
  const [subMerchants, setSubMerchants] = useState(subMerchantData);
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');
  const [selectedSubMerchant, setSelectedSubMerchant] = useState<typeof subMerchantData[0] | null>(null);
  const [showDetails, setShowDetails] = useState(false);
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [confirmAction, setConfirmAction] = useState<{ type: string; merchant: typeof subMerchantData[0] | null }>({ type: '', merchant: null });
  ...
  const handleAction = (type: string, merchant: typeof subMerchantData[0]) => {
